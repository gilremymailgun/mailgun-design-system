...
interface NavItemProps {
  ...
  hasChevron?: boolean;
  isExternal?: boolean;
  /** Icon-only rendering, no label/chevron, for collapsed nav mode. */
  collapsed?: boolean;
  onClick?: () => void;
}
...
export const NavItem = ({
  ...
  return (
    <div className={classes} onClick={onClick} role="button" tabIndex={0} title={collapsed ? label : undefined}>
      <div className="nav-item__left">
        <div className="nav-item__icon">{icon}</div>
        {!collapsed && <span className="nav-item__label">{label}</span>}
      </div>
      {!collapsed && (hasChevron || isExternal) && (
